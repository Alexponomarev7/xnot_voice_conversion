...
def sample_from_tensor(tensor: Tensor, batch_size: int = 64) -> Tensor:
    indices = random.choices(range(len(tensor.shape[0])), k=batch_size)
...
class XNot:
    ...
    def fit(self,
        ...
        for _ in tqdm(range(max_steps)): # step
            for _ in range(t_iters): # t_iter
                X = torch.tensor(sample_from_tensor(query_seq, batch_size=batch_size), device=self.device)
                T_loss = cost_fn(X, self.T(X)).mean() - self.f(self.T(X)).mean()
                T_opt.zero_grad(); T_loss.backward(); T_opt.step()

            # f optimization
            self.T.eval(); self.f.train(True)
            X = sample_from_tensor(query_seq, batch_size=batch_size)
            Y = sample_from_tensor(matching_set, batch_size=batch_size)
